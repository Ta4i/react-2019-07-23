...
describe('At starts', function () {
  ...
  describe('when click on Open menu button two times', function () {
    it('should close menu', function () {
      const wrapper = mount(<App restaurants={restaurants} />)
      const id = restaurants[0].id
      const openMenuButton = wrapper.find(`button[data-autoid="OPEN_MENU_ITEM_${id}"]`)
  ...
  describe('when click on add dish button', function () {
    it('should increase amount of dishes', function () {
      ...
      wrapper.find(`button[data-autoid="OPEN_MENU_ITEM_${restaurantId}"]`)
        .simulate('click')

      wrapper.find(`button[data-autoid="ADD_DISH_${dishId}"]`)
        .simulate('click')
        .simulate('click')
        .simulate('click')

      expect(wrapper.find(`span[data-autoid="DISH_AMOUNT_${dishId}"]`).text()).toEqual('3')
    });
  });
  ...
  describe('when click on remove dish button', function () {
    it('should decrease amount of dishes until 0', function () {
      ...
      wrapper.find(`button[data-autoid="REMOVE_DISH_${dishId}"]`)
        .simulate('click')
        .simulate('click')
        .simulate('click')
        .simulate('click')
      ...
      expect(wrapper.find(`span[data-autoid="DISH_AMOUNT_${dishId}"]`).text()).toEqual('0')
    });
  });
